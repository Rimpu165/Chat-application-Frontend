...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-chat-bg text-chat-text selection:bg-blue-500/30">
      ...
      {/* Cover Backdrop */}
      <div className="relative h-40 md:h-52 w-full overflow-hidden bg-chat-surface border-b border-chat-border">
        {coverUrl ? (
          <img src={coverUrl} alt="Cover" className="h-full w-full object-cover opacity-60" />
        ) : (
          <div className="h-full w-full bg-linear-to-br from-blue-600/20 via-purple-600/20 to-chat-bg" />
        )}
              ...
              {activeTab === "about" ? (
                ...
                >
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-2 text-chat-muted">
                       <div className="flex items-center gap-3 bg-chat-bg p-4 rounded-2xl border border-chat-border text-chat-muted focus-within:border-chat-accent/40 transition-all">
                          <Camera className="h-4 w-4" />
                          <input 
                            placeholder="Instagram username" 
                            className="bg-transparent text-sm focus:outline-none w-full text-chat-text" 
                ...
                >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {user.gallery && user.gallery.length > 0 ? user.gallery.map((img: string, idx: number) => (
                      <div key={idx} className="group relative aspect-square overflow-hidden rounded-3xl bg-chat-raised border border-chat-border">
             ...
             <div className="rounded-[40px] border border-chat-border bg-chat-surface/60 p-8 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div className="bg-chat-bg p-4 rounded-3xl border border-chat-border">
                      <p className="text-[10px] font-bold text-chat-muted uppercase">Friends</p>
                      <p className="text-2xl font-black">{user.friends?.length || 0}</p>
                   </div>
                   <div className="bg-chat-bg p-4 rounded-3xl border border-chat-border">
                      <p className="text-[10px] font-bold text-chat-muted uppercase">Gallery</p>
